...
registerChannelAdapter('whatsapp', {
  factory: () => {
    ...
    /** Download media from an inbound message for host-side group materialization. */
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    > {
      ...
      for (const { key, type, ext } of mediaTypes) {
        ...
        const media = normalized[key] as { fileName?: string; mimetype?: string; fileLength?: number | LongLike };
        const docFilename = media.fileName;
        const filename = docFilename || `${type}-${Date.now()}${ext}`;
        const mimeType = media.mimetype ?? null;
        try {
          const buffer = await downloadMediaMessage(msg, 'buffer', {});
          results.push({
            id: `${type}-${msg.key.id || Date.now()}`,
            type,
            name: filename,
            mimeType,
          ...
          results.push({
            ...
            name: filename,
            mimeType,
            size: coerceMediaSize(media.fileLength),
    ...
    function coerceMediaSize(value: number | LongLike | undefined): number {
