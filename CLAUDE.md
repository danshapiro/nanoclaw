...
the shim, and package installs must fail closed if they create another
executable `gws` anywhere on `PATH` other than `/usr/local/bin/gws`, or at the
explicit `/pnpm/gws` bypass path.
