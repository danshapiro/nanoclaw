...
- [ ] **Step 0: Verify toolchain canon and clean start (drift already remediated during plan review)**

Load-bearing validation (2026-08-07) proved an npm flat install had drifted the toolchain (prettier 3.9.6 / vitest 4.1.10 / js-yaml 4.3.1 vs the `pnpm-lock.yaml` canon of 3.8.2 / 4.1.4 / 4.1.1; typescript 5.9.3 matched) and the drifted prettier had churned 22 tracked files under `src/` (formatting-only union-pipe rewraps, ~33 insertions / 74 deletions, zero semantic edits) plus dropped an untracked npm `package-lock.json`. During plan review (fresh-eyes iteration 1, same day) that state was fully remediated on this branch: the churn and `package-lock.json` were removed (`git reset --hard` to the plan-hardening commit) and `node_modules` was realigned via `pnpm install --frozen-lockfile` (verified: `pnpm exec prettier --version` → 3.8.2, clean tree). This step VERIFIES the remediated state still holds and re-applies it if anything regressed:

```bash
cd /home/dan/code/nanoclaw-frontmatter-py312/.worktrees/frontmatter-py312
git status --porcelain           # expect: EMPTY output (clean tree; no package-lock.json anywhere)
corepack enable                  # Node 22 ships corepack; provides pnpm
pnpm install --frozen-lockfile   # idempotent realign to pnpm-lock.yaml; needs network; changes no lockfile
pnpm exec prettier --version     # expect: 3.8.2 (guards the pre-commit hook against re-churning src/)
git status --porcelain           # expect: still EMPTY
```

If the FIRST `git status --porcelain` is not empty: the known-drift signature is formatting-only ' M src/...' lines plus '?? package-lock.json' — confirm via `git diff` that the changes are formatting-only union-pipe rewraps (added/removed identifiers or logic: STOP, do not restore, investigate first), then run `git restore src/` and `rm -f package-lock.json` (a generated npm artifact; never commit it) and re-run the block from the top. Any OTHER dirty state: STOP and investigate. If `pnpm install --frozen-lockfile` fails (network/policy) and `pnpm exec prettier --version` reports 3.9.x, the pre-commit hook (`prettier --write \"src/**/*.ts\"`) will rewrite the 22 known files as UNSTAGED churn on every commit — the Task-1 target files are provably formatted identically under both prettier versions, so proceed, but after EVERY commit run `git restore src/` to discard the hook churn, never `git add` those files, verify each commit with `git log -1 --stat`, and record the deviation for Task 3 Step 2.
...
**Interfaces:**
- Consumes: the two commits from Tasks 1–2 on `fix/frontmatter-py312`; the three `docs:` plan commits (already on the branch).
...
```
Expected: status output is EMPTY — the formatter churn and `package-lock.json` were removed from this branch during plan review (fresh-eyes iteration 1) and Task 1 Step 0 verified the clean state. If `package-lock.json` reappears, something ran `npm install`: `rm -f` it (never commit it). Any other modification is unexpected — stop and investigate (exception: if Step 0's documented pnpm-failure fallback was taken, exactly the 22 known churn files may appear as unstaged modifications; verify they are exactly those, `git restore src/`, and confirm no commit includes them). The log shows exactly five commits — the plan commit (`docs: add implementation plan for frontmatter-py312`), the plan-hardening commit (`docs: harden frontmatter-py312 plan with load-bearing validation findings`), the plan-reconciliation commit (`docs: reconcile frontmatter-py312 plan with remediated branch state`), the Fix 1 commit (`fix(yente): ...`), and the Fix 2 commit (`feat(container): ...`). Every commit in the range must be either a `docs:` commit touching only `docs/plans/` or one of those two fix commits; any commit touching `src/` formatting or any lockfile is a hard stop.
...
6. **Plan-reconciliation addendum (2026-08-07, workflow Stage 3 fresh-eyes iteration 1):** an automation pass between plan hardening and review had committed the Step-0 churn (prettier-3.9.6 rewraps of 22 `src/` files) and the npm `package-lock.json` as two stray commits, falsifying Step 0's premises, Task 3 Step 2's gates (clean-but-for-lockfile tree; "exactly four commits"), and the endgame's pnpm-canonical push discipline. Remediation performed on the branch: `git reset --hard` to the plan-hardening commit (dropping both stray commits — they contained only the artifacts Step 0 existed to discard) and `pnpm install --frozen-lockfile` realign (prettier verified back at 3.8.2, clean tree). Plan edits re-reviewed against this checklist: Task 1 Step 0 rewritten as verify-then-repair with complete commands and expected outputs (clean-tree expectation, prettier-version guard, drift-recurrence and no-network fallbacks — all commands executed successfully during remediation); Task 3 Step 2's commit count updated four → five (superseding item 5c's "four commits" reference) plus a semantic guard that every range commit is a `docs:` plans-only commit or one of the two fix commits; Task 3 Interfaces consumes-line updated to the three `docs:` commits. No placeholders introduced; every command carries an expected output; no test design or verification content changed.
