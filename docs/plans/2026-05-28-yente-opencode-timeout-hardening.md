...
- `buildOpenCodeConfig()` must raise or disable OpenCode's model-provider request timeout for long Yente turns so NanoClaw's liveness pump is not undercut by a hidden 5-minute provider request timeout. In SDK 1.15.10 `Config.provider` is keyed by provider name and the field is `provider[<activeProvider>].options.timeout` — there is no top-level `Config.options.timeout`. The active provider is `process.env.OPENCODE_PROVIDER || 'anthropic'`; set its `options.timeout` to `0`/`false`/the largest supported value. Emitting `{ provider: { options: { timeout } } }` is a bug (it creates a provider literally named `options` and leaves the real abort in place). Tests must assert the value resolves under the ACTIVE provider name, not merely that a `timeout` key exists somewhere, and the field name must come from the SDK-surface probe rather than a hard-coded belief.
...
- Modify `proxy.go`:
  ...
  - Task 4B (signed side-effect channel): generate a unique `audit_id` per request; on a successful API-effect mutation, Ed25519-sign the canonical payload with the key from `GWS_SIDE_EFFECT_SIGN_KEY_FILE` and return `X-GWS-Side-Effect-Signature` + `X-GWS-Side-Effect-Payload`; append an entry to a small append-only audit store (file/SQLite) keyed by `audit_id`/`input_id`/`route_key`/time for the crash-window discovery fallback. Uses Go stdlib `crypto/ed25519`.
...
Neither `gws-skill` nor `summarize-dnd` has an `AGENTS.md`/`CLAUDE.md`, so the "read the repo's AGENTS.md first" rule cannot be satisfied and the `.venv-wsl` interpreter the later Python steps invoke does not exist. Discover the real invocation before writing tests:
...
- `OPENCODE_MODEL_PROVIDER_TIMEOUT_MS=0` or the largest value supported by the active OpenCode config, applied under the active provider name (`provider[OPENCODE_PROVIDER].options.timeout`), to avoid a hidden 5-minute request abort.
...
- The proxy appends an entry to a small append-only audit store (file or SQLite) keyed by `audit_id`/`input_id`/`route_key`/`occurred_at`, and a query helper can find a successful `drafts.create` by `input_id`/`route_key`/time window (the crash-window discovery fallback).
...
- The append is atomic (temp+rename or `flock`). If the API mutation succeeds but the append fails, the shim emits a structured partial-success error (audit id, no raw body) instead of ordinary stdout success.
...
In `proxy.go`, load the Ed25519 private key from `GWS_SIDE_EFFECT_SIGN_KEY_FILE` (root-owned) at startup. On a successful API-effect mutation, build the canonical payload `{audit_id, service, method, request_class, api_effect, operation_succeeded, occurred_at, result_digest}`, sign it with `crypto/ed25519`, and set `X-GWS-Side-Effect-Signature`/`X-GWS-Side-Effect-Payload`. Append the same fields (+ `input_id`/`route_key` if the shim forwarded them) to an append-only audit store and expose a query for the discovery fallback. If no key is configured, skip signing and the audit store, emit classification headers only, and log one warning. Do not change admission/policy/rate-limit/execution.

- [ ] **Step 7: Implement NanoClaw GWS shim ledger writing**

In `container/shim/gws`, append JSONL only after HTTP 200 with `X-GWS-Api-Effect: true`, `X-GWS-Operation-Succeeded: true`. Stage the `X-GWS-Side-Effect-Signature`/`-Payload` verbatim (do not verify in sh), read `inputId`/`routeKey` from `/workspace/.active-input.json`, sanitize args (preserve method/resource, redact long body fields), and append atomically (temp+rename). On append failure after API success, emit a recoverable partial-success error with the audit id and no raw body.
