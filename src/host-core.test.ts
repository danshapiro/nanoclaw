...
describe('session manager', () => {
  ...
  it('materializes inbound channel attachments to the group workspace', () => {
    const { session } = resolveSession('ag-1', 'mg-1', null, 'shared');
    ...
    expect(parsed.text).toContain('/workspace/agent/attachments/discord/msg-attachment/att-1-screen-shot.png');
    expect(parsed.text).toContain('Content-Type: image/png');
    expect(parsed.attachments[0].workspacePath).toBe(
      '/workspace/agent/attachments/discord/msg-attachment/att-1-screen-shot.png',
    );
