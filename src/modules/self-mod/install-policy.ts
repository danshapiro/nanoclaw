...
const DEFAULT_SKILLS_ROOT = '/srv/nanoclaw/shared/skills';
...
export function evaluateInstallPackagesRequest(input: InstallPolicyInput): InstallPolicyDecision {
  if (input.purpose === 'deployed_skill_dependency') return reject();
  ...
  for (const hint of loadSkillHints(
    input.skillsRoot ?? process.env.NANOCLAW_MANAGED_SKILLS_ROOT ?? DEFAULT_SKILLS_ROOT,
  )) {
