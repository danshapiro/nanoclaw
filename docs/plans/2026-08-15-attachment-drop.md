...
- Round 10 (commit D1): the sibling sweep was corrected — gate flipped to `session.created_at >= message.timestamp` (a first-message-created session stamps host receipt time and still passes), agent-shared sweeps the agent's full session history (canonical per-agent session ignores mg/thread), unreadable/missing archived sibling DBs are skipped with a warning instead of aborting routing, and ordinary live traffic never sweeps (bounded cost); includes the exact gate expression and candidate rule.
- Round 11 (this commit): archived-sibling identity mismatches are ignored instead of thrown (sibling rows are not write-target candidates; agent-shared sweeps span messaging groups), and sibling candidates are processed in deterministic (created_at, id) order.
